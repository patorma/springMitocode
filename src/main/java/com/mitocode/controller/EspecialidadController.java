...
public class EspecialidadController {
	...
	public ResponseEntity<List<EspecialidadDTO>> listar() throws Exception{
		//hay que ver como de eta lista especialidad termino llegando
		//a especialidad DTO con mapper (ver declaracion clase MapperConfig)
		//con stream() se transforma a a EspecialidadDTO
	...
	public ResponseEntity<EspecialidadDTO> listarPorId(@PathVariable("id") Integer id) throws Exception{
		...
		//debo tratar de obtener un especialidaddto
		EspecialidadDTO dto = mapper.map(obj, EspecialidadDTO.class);//source y fuente de destino
		return new ResponseEntity<>(dto,HttpStatus.OK);
	}
	
	/*@PostMapping
	public ResponseEntity<Especialidad> registrar(@RequestBody Especialidad especialidad) throws Exception{
	...
	@PutMapping
	public ResponseEntity<Especialidad> modificar(@Valid @RequestBody EspecialidadDTO dtoRequest) throws Exception{
		Especialidad obj = service.listarPorId(dtoRequest.getIdEspecialidad());
		
		if(obj == null) {
			throw new ModeloNotFoundException("ID NO ENONTRADO " + dtoRequest.getIdEspecialidad());
		}
		
		Especialidad pac = mapper.map(dtoRequest,Especialidad.class);
		Especialidad especialidad = service.modificar(pac);
	...
	//objetivo listar un especialidad por su id
	...
	public EntityModel<EspecialidadDTO> listarHateoas(@PathVariable("id") Integer id) throws Exception{
		...
		// generamos un localhost:8080/especialidads/{id} el id es dinamico
		...
		recurso.add(link1.withRel("especialidad-info1"));
		//recurso.add(link2.withRel("especialidad-info2"));
